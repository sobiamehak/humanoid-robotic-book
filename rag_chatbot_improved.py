...
@function_tool
async def classify_and_respond(query: str) -> str:
    """
    Classify the query and provide appropriate response without using RAG for greetings/off-topic.
    """
    if is_greeting_or_off_topic(query):
        ...
        elif query.lower().strip() in ['yes', 'no', 'maybe', 'ok', 'okay', 'sure', 'thanks', 'thank you']:
        ...
        # For book questions, retrieve context and let the agent handle the response
        context = await retrieve_book_context(query)
        return context

# Main agent that handles both greetings and book questions appropriately
main_agent = Agent(
    name="Smart RAG Chatbot",
    instructions="Use the classify_and_respond tool to handle user queries. "
                "For greetings and off-topic queries (like 'hi', 'hello', 'yes', 'no'), "
                "respond briefly with a friendly message inviting questions about the book. "
                "For actual book questions, use RAG retrieval to get relevant context, "
                "then provide accurate answers citing chapters from the book. "
                "Always respond concisely and avoid hallucinations.",
    tools=[classify_and_respond, retrieve_book_context],
    model=model
)
